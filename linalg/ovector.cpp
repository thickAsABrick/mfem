...
void OccaVector::SetSize(occa::device device, const int64_t size_,
                         const void *src)
{
   size = size_;
...
OccaVectorRef OccaVector::GetRange(const uint64_t offset,
                                   const uint64_t entries) const
{
   OccaVectorRef ret;
   OccaVector &v = ret.v;
...
void OccaVector::SetSubVectorComplement(const Array<int> &dofs,
                                        const double val)
{
   const int dofCount = dofs.Size();
   if (dofCount == 0)
         ...
         OccaVector &out)
{
   ...
   occa::kernel kernel = builder.build(out.GetDevice());
   kernel((int) out.Size(), alpha, beta, out.GetData(), v1.GetData(),
          v2.GetData());
}
///===================================
